...
install_ss_panel_mod_v3(){
	...
	num=$1
	if [ "${num}" != "1" ]; then
  	  wget -c --no-check-certificate https://raw.githubusercontent.com/cangshuking/ss-panel-and-ss-py-mu_cangshu/master/lnmp1.4.zip && unzip lnmp1.4.zip && rm -rf lnmp1.4.zip && cd lnmp1.4 && chmod +x install.sh && ./install.sh lnmp
	fi
	cd /home/wwwroot/
	cp -r default/phpmyadmin/ .
	cd default
	rm -rf index.html
	git clone https://github.com/cangshuking/ss-panel-v3-mod_cangshu.git tmp && mv tmp/.git . && rm -rf tmp && git reset --hard
	...
	wget -N -P  /usr/local/nginx/conf/ --no-check-certificate https://raw.githubusercontent.com/cangshuking/ss-panel-and-ss-py-mu_cangshu/master/nginx.conf
...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/FaceTheWorld233/ssr-script-node-update/master/libsodium-1.0.13.tar.gz'
	else
...
install_centos_ssr(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/FaceTheWorld233/ssr-script-node-update/master/supervisord.conf
	supervisord
	#第一次安装
	python_test
...
install_node_glzjin(){
	...
	# MYSQL_HOST
	sed -i '24d' /root/shadowsocks/userapiconfig.py
	sed -i "24i\MYSQL_HOST = '${MYSQL_HOST}'" /root/shadowsocks/userapiconfig.py
	# MYSQL_USER
	sed -i '26d' /root/shadowsocks/userapiconfig.py
	sed -i "26i\MYSQL_USER = '${MYSQL_USER}'" /root/shadowsocks/userapiconfig.py
	# MYSQL_PASS
	sed -i '27d' /root/shadowsocks/userapiconfig.py
	sed -i "27i\MYSQL_PASS = '${MYSQL_PASS}'" /root/shadowsocks/userapiconfig.py
	# MYSQL_DB
	sed -i '28d' /root/shadowsocks/userapiconfig.py
	sed -i "28i\MYSQL_DB = '${MYSQL_DB}'" /root/shadowsocks/userapiconfig.py
	#iptables
	iptables -F
	iptables -X  
	iptables -I INPUT -p tcp -m tcp --dport 22:65535 -j ACCEPT
...
install_panel_and_node(){
  ...
  sed -i '$a [program:ssr]\ncommand = python /root/shadowsocks/server.py\nuser = root\nautostart = true\nautorestart = true' /etc/supervisord.conf
	...
	echo "# fork: https://github.com/mmmwhy/ss-panel-and-ss-py-mu     #"
	echo "# Author: 91vps, dumplin                                    #"
	echo "# Please choose the server you want                         #"
	echo "# 1  SS-V3_mod_panel and node One click Install             #"
	echo "# 2  SS-node_webapi One click Install                       #"
